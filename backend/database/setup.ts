...
async function setupOperators() {
  try {
    ...
    if (!existingOperator) {
      ...
      const _products = await fetch('https://configs.cashflowcasino.com/house/bonuses.seed.json');
  ...
  try {
    ...
    if (result) {
      ...
      if (userRecord) {
        ...
        await db.update(users).set({ emailVerified: true }).where(eq(users.id, userRecord.id));
...
const setupUsers = async () => {
  ...
  try {
    ...
    console.log(`Found ${affiliateUserCount} existing users with affiliate roles`);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const name = randUserName({ withAccents: false }).replaceAll('-', '').replaceAll('_', '');
      ...
      try {
        ...
        if (result) {
          ...
          if (userRecord) {
            ...
            await db.update(users).set({ createdAt }).where(eq(users.id, userRecord.id));
