...
import { players, operators, games, transactions } from './core.schema'; // Import core tables
import {
  depositStatusEnum,
...
export const gameSessions = pgTable(
  ...
  (table) => ({
    ...
    authSessionIdx: index('game_sessions_auth_session_idx').on(table.authSessionId),
  }),
);
...
export type TGameSessionsSelect = typeof gameSessions.$inferSelect & TGameSessions;
...
export const jackpots = pgTable(
  'jackpots',
  {
    ...
    lastWonByPlayerId: text('last_won_by_player_id').references(() => players.id),
    totalContributions: cents('total_contributions').default(0), // Added tracking
    totalWins: cents('total_wins').default(0), // Added tracking
    createdAt: timestamp('created_at', { mode: 'date', precision: 3 }).defaultNow().notNull(), // Add timestamps
...
export const jackpotContributions = pgTable(
  'jackpot_contributions',
  {
    ...
    betTransactionId: text('bet_transaction_id').references(() => transactions.id), // Link to bet TXN
...
export const ZJackpotContributionsSelectSchema = createSelectSchema(jackpotContributions);
export const ZJackpotContributionsUpdateSchema = createUpdateSchema(jackpotContributions);
export type TJackpotContributions = z.infer<typeof jackpotContributions>;
export type TJackpotContributionsSelect = typeof jackpotContributions.$inferSelect &
  TJackpotContributions;
...
export const jackpotWins = pgTable(
  'jackpot_wins',
  {
    ...
    winTransactionId: text('win_transaction_id').references(() => transactions.id), // Link to win TXN
...
export type TReferralCodesSelect = typeof referralCodes.$inferSelect & TReferralCodes;
...
export type TKycSubmissionsSelect = typeof kycSubmissions.$inferSelect & TKycSubmissions;
...
export const kycDocuments = pgTable(
  ...
  (table) => ({
    ...
    playerTypeIdx: index('kyc_documents_player_type_idx').on(table.playerId, table.type),
  }),
);
...
export type TKycDocumentsSelect = typeof kycDocuments.$inferSelect & TKycDocuments;
...
export type TPasswordLogsSelect = typeof passwordLogs.$inferSelect & TPasswordLogs;
...
export const ZOperatorSwitchHistoriesSelectSchema = createSelectSchema(operatorSwitchHistories);
export const ZOperatorSwitchHistoriesUpdateSchema = createUpdateSchema(operatorSwitchHistories);
export type TOperatorSwitchHistories = z.infer<typeof operatorSwitchHistories>;
export type TOperatorSwitchHistoriesSelect = typeof operatorSwitchHistories.$inferSelect &
  TOperatorSwitchHistories;
...
export const operatorSettlements = pgTable(
  'operator_settlements',
  {
    ...
    operatorId: text('operator_id')
      .notNull()
      .references(() => operators.id),
    periodStartDate: timestamp('period_start_date', { mode: 'date', precision: 3 }).notNull(),
    periodEndDate: timestamp('period_end_date', { mode: 'date', precision: 3 }).notNull(),
...
export const ZOperatorSettlementsSelectSchema = createSelectSchema(operatorSettlements);
export const ZOperatorSettlementsUpdateSchema = createUpdateSchema(operatorSettlements);
export type TOperatorSettlements = z.infer<typeof operatorSettlements>;
export type TOperatorSettlementsSelect = typeof operatorSettlements.$inferSelect &
  TOperatorSettlements;
...
export const ZLoyaltyFundTransactionsSelectSchema = createSelectSchema(loyaltyFundTransactions);
export const ZLoyaltyFundTransactionsUpdateSchema = createUpdateSchema(loyaltyFundTransactions);
export type TLoyaltyFundTransactions = z.infer<typeof loyaltyFundTransactions>;
export type TLoyaltyFundTransactionsSelect = typeof loyaltyFundTransactions.$inferSelect &
  TLoyaltyFundTransactions;
...
export const gameSessionsRelations = relations(gameSessions, ({ one }) => ({
  ...
  // authSession: one(authSessions, { ... }) // Relation to main auth session if needed
}));
...
export const jackpotsRelations = relations(jackpots, ({ one, many }) => ({
  ...
  // contributions: many(jackpotContributions), // If needed
  // wins: many(jackpotWins), // If needed
}));

export const jackpotContributionsRelations = relations(jackpotContributions, ({ one }) => ({
  jackpot: one(jackpots, {
    fields: [jackpotContributions.jackpotGroup], // Assuming group is the link
    references: [jackpots.group], // Assuming group is unique key
  }),
  player: one(players, {
...
export const jackpotWinsRelations = relations(jackpotWins, ({ one }) => ({
  jackpot: one(jackpots, {
    fields: [jackpotWins.jackpotGroup], // Assuming group is the link
    references: [jackpots.group], // Assuming group is unique key
  }),
  winner: one(players, {
...
export const kycSubmissionRelations = relations(kycSubmissions, ({ one, many }) => ({
  player: one(players, {
...
export const passwordLogRelations = relations(passwordLogs, ({ one }) => ({
  user: one(players, {
...
export const operatorSwitchHistoriesRelations = relations(operatorSwitchHistories, ({ one }) => ({
  player: one(players, {
...
export const operatorSettlementsRelations = relations(operatorSettlements, ({ one }) => ({
  operator: one(operators, {
...
export const loyaltyFundTransactionsRelations = relations(loyaltyFundTransactions, ({ one }) => ({
  operator: one(operators, {
...
//   // Define relations from settings to other config tables if needed
//   // e.g., commissionRates: many(commissions)
// }));
...
export const VipConfigSchema = z.object({
  ...
  vipRanks: z.array(VipRankSchema),
});
...
export const SettinsSchema = z.object({
  id: z.string(),
  name: z.string(),
