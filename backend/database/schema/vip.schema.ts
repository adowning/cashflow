...
// Remove circular imports - tables are imported in relations.ts
import { createSelectSchema, createUpdateSchema } from "drizzle-zod";
import { z } from "zod";
import { players } from "./core.schema";
...
export const vipLevels = pgTable("vip_levels", {
  ...
  spinBonusMultiplier: doublePrecision("spin_bonus_multiplier_id")
    .default(0.1)
    .notNull(),
...
export const playerBonuses = pgTable(
  ...
  (table) => ({
    playerBonusIdx: index("player_bonuses_player_bonus_idx").on(
      table.playerId,
      table.bonusId
    ),
...
export type TPlayerBonusesSelect = typeof playerBonuses.$inferSelect &
  TPlayerBonuses;
...
//     affiliateName: text("affiliate_name").notNull().unique(),
...
//   },
//   (table) => ({
//     referralCodeIdx: uniqueIndex("affiliates_ref_code_idx").on(
//       table.referralCode
//     ),
...
export const affiliateLogs = pgTable(
  "affiliate_logs",
  {
    ...
    invitorId: text("invitor_id")
      .notNull()
      .references(() => players.id), // Affiliate who earned
    childId: text("child_id").references(() => players.id), // Player who bet
  ...
  (table) => ({
    invitorDateIdx: index("affiliate_logs_invitor_date_idx").on(
      table.invitorId,
      table.createdAt
    ),
...
export type TAffiliateLogsSelect = typeof affiliateLogs.$inferSelect &
  TAffiliateLogs;
...
export const affiliatePayouts = pgTable(
  ...
  (table) => ({
    affiliateWeekIdx: index("affiliate_payouts_week_idx").on(
      table.affiliateId,
      table.weekStart
    ),
...
export const ZAffiliatePayoutsSelectSchema =
  createSelectSchema(affiliatePayouts);
export const ZAffiliatePayoutsUpdateSchema =
  createUpdateSchema(affiliatePayouts);
export type TAffiliatePayouts = z.infer<typeof affiliatePayouts>;
export type TAffiliatePayoutsSelect = typeof affiliatePayouts.$inferSelect &
  TAffiliatePayouts;
...
export const vipCashbacks = pgTable(
  ...
  (table) => ({
    playerPeriodIdx: index("vip_cashbacks_player_period_idx").on(
      table.playerId,
      table.periodStart
    ),
  })
...
export type TVipCashbacksSelect = typeof vipCashbacks.$inferSelect &
  TVipCashbacks;
...
export const ZVipLevelUpBonusesSelectSchema =
  createSelectSchema(vipLevelUpBonuses);
export const ZVipLevelUpBonusesUpdateSchema =
  createUpdateSchema(vipLevelUpBonuses);
export type TVipLevelUpBonuses = z.infer<typeof vipLevelUpBonuses>;
export type TVipLevelUpBonusesSelect = typeof vipLevelUpBonuses.$inferSelect &
  TVipLevelUpBonuses;
...
export type TVipSpinRewardsSelect = typeof vipSpinRewards.$inferSelect &
  TVipSpinRewards;
