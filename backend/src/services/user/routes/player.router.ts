...
export const updateUsernameRoute = createRoute({
	...
	responses: {
		...
		400: {
			description:
				"Bad Request - Username already taken, invalid format, or validation failed.",
			content: { "application/json": { schema: ErrorResponseSchema } },
		},
		500: {
...
export const updateCurrencyRoute = createRoute({
	...
	responses: {
		...
		400: {
			description:
				"Bad Request - Invalid currency code or currency not supported.",
			content: { "application/json": { schema: ErrorResponseSchema } },
		},
		500: {
			description:
				"Internal Server Error - Failed to update currency preference.",
			content: { "application/json": { schema: ErrorResponseSchema } },
		},
	},
});
...
export const updateAvatarRoute = createRoute({
	...
	description:
		"Updates the avatar/profile picture for the authenticated player.",
	request: {
		body: {
			content: {
				"application/json": {
	...
	responses: {
		...
		400: {
			description:
				"Bad Request - Invalid avatar URL or image format not supported.",
			content: { "application/json": { schema: ErrorResponseSchema } },
		},
		500: {
...
export const createPlayerBonusRoute = createRoute({
	...
	responses: {
		...
		400: {
			description:
				"Bad Request - Invalid bonus type, amount, or bonus conditions not met.",
			content: { "application/json": { schema: ErrorResponseSchema } },
		},
		500: {
...
export const claimBonusRoute = createRoute({
	...
	description:
		"Claims a specific bonus that is available to the authenticated player.",
	request: {
		params: z.object({
	...
	responses: {
		...
		400: {
			description:
				"Bad Request - Bonus not found, already claimed, or conditions not met.",
			content: { "application/json": { schema: ErrorResponseSchema } },
		},
		500: {
...
export const generateRoutes = (router: AppOpenAPI) =>
{
	...
	router.openapi(
		getPlayerTransactionsRoute,
		controller.getPlayerTransactions,
	);
	...
	router.openapi(
		createPlayerWithdrawRoute,
		controller.getPlayerWithdraw,
	);
