...
export const updateAffiliate = catchAsyncV2(async (c: Context) => {
	...
	const affiliate = await affiliateService.patchUpdate(
		{ _id: affiliateId },
		data,
	);
...
export const getDashboardAnalysis = catchAsyncV2(async (c: Context) => {
	const affiliateId = c.get("affiliate")._id;
	const data = await affiliateService.getAnalysis(
		affiliateId,
		await c.req.json(),
	);
...
export const getDashboardChildren = catchAsyncV2(async (c: Context) => {
	const affiliateId = c.get("affiliate")._id;
	const data = await affiliateService.getDashboardChildren(
		affiliateId,
		await c.req.json(),
	);
	return c.json(data);
...
export const getChildrenAffiliate = catchAsyncV2(async (c: Context) => {
	const affiliateId = c.get("affiliate")._id;
	const data = await affiliateService.getChildrenAffiliate(
		String(affiliateId),
		await c.req.json(),
	);
	return c.json(data);
...
export const getAffiliateUsers = catchAsyncV2(async (c: Context) => {
	const affiliateId = c.get("affiliate")._id;
	const data = await affiliateService.getAffiliateUsers(
		String(affiliateId),
		await c.req.json(),
	);
	return c.json(data);
