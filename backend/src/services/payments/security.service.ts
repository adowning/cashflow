...
import { deposits, withdrawals, players, } from '../../database/schema'
import type { User, Deposits, Withdrawal } from '../../database/interfaces'
...
): Promise<VelocityCheck>
{
  ...
  const currentAmount = recentDeposits.reduce((sum, d) => sum + Number(d.amount), 0)
...
): Promise<VelocityCheck>
{
  ...
  const currentAmount = recentWithdrawals.reduce((sum, w) => sum + Number(w.amount), 0)
