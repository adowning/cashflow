...
import { createBalanceForNewUser, getDetailedBalance } from '../../services/balance-management.service';
import { getGeoInfo } from '../../utils/getCountry';
import { getAffiliateByReferralCode, getReferralCodeByCode } from '../../services/ggr.service';
import { createUser, emailTaken, emailTaken, getUserByEmail, getUserByPhone, getUserByUsername, usernameTaken } from './user.service';
...
export const adminLogin = catchAsyncV2(async (c: Context) => {
  ...
  const token = jwt.sign({ id: user._id, username: user.username }, configurationManager.jwt.secret);
  const expiredTime = moment().add(configurationManager.jwt.accessExpirationMinutes, 'minutes').toDate();
