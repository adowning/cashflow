...
// import db from '@/db'
// import { deposits, withdrawals, } from '@/types'
// import { and, desc, eq, gte, lte, sql } from 'drizzle-orm'
// import { cleanupExpiredDeposits } from '../services/deposit.service.js'
// import { processWithdrawalAction } from '../services/withdrawal.service.js'
...
//     // Apply filters
//     if (filter.status) {
//       query = query.where(eq(deposits.status, filter.status))
//     }

//     if (filter.dateFrom) {
//       query = query.where(gte(deposits.createdAt, filter.dateFrom))
//     }

//     if (filter.dateTo) {
//       query = query.where(lte(deposits.createdAt, filter.dateTo))
//     }

//     if (filter.amountMin) {
//       query = query.where(sql`${deposits.amount} >= ${filter.amountMin}`)
//     }

//     if (filter.amountMax) {
//       query = query.where(sql`${deposits.amount} <= ${filter.amountMax}`)
//     }
...
//     if (!total[0]) throw new Error('no deposits')
...
//  */
// async function getDailyVolume(startDate: Date, days: number): Promise<Array<{
//   date: string
//   deposits: number
//   withdrawals: number
// }>>
...
//     if (!_deposits[0] || !_withdrawals[0]) throw new Error('no deposits or withdrawls')
