...
export const GameSchema = z
  .object({
    ...
    name: z.string().openapi({ description: 'Game name', example: 'Slot Machine Deluxe' }),
    title: z.string().nullable().openapi({ description: 'Game title', example: 'Deluxe Slots' }),
    description: z.string().nullable().openapi({
      description: 'Game description',
      example: 'An exciting slot game',
    }),
    category: z.string().openapi({ description: 'Game category', example: 'slots' }),
    tags: z.string().nullable().openapi({ description: 'Game tags', example: 'popular,new' }),
    ...
    developer: z.string().openapi({ description: 'Game developer', example: 'GameProvider Inc' }),
    providerId: z
      .string()
      .nullable()
    ...
    isFeatured: z.boolean().openapi({ description: 'Whether game is featured', example: true }),
    isActive: z.boolean().openapi({ description: 'Whether game is active', example: true }),
...
export const ProviderSchema = z
  .object({
    ...
    name: z.string().openapi({ description: 'Provider name', example: 'GameProvider Inc' }),
    code: z.string().openapi({ description: 'Provider code', example: 'GP' }),
    isActive: z.boolean().openapi({ description: 'Whether provider is active', example: true }),
    gameCount: z.number().openapi({ description: 'Number of games', example: 150 }),
...
export const SportSchema = z
  .object({
    ...
    name: z.string().openapi({ description: 'Sport name', example: 'Football' }),
    code: z.string().openapi({ description: 'Sport code', example: 'FB' }),
    isActive: z.boolean().openapi({ description: 'Whether sport is active', example: true }),
    eventCount: z.number().openapi({ description: 'Number of events', example: 25 }),
  })
  .openapi('Sport');
...
export const GameDetailResponseSchema = z
  .object({
    ...
    targetRtp: z.number().nullable().openapi({ description: 'Target RTP', example: 96.5 }),
    isFeatured: z.boolean().openapi({ description: 'Featured status', example: true }),
...
export const RecentBigWinResponseSchema = z
  .array(
    z.object({
      ...
      gameName: z.string().openapi({ description: 'Game name', example: 'Slot Machine Deluxe' }),
      playerName: z.string().openapi({ description: 'Player name', example: 'john_doe' }),
      amount: z.number().openapi({ description: 'Win amount', example: 1500.5 }),
...
createRoute({
  ...
  tags: ['Games'],
  responses: {
    [HttpStatusCodes.OK]: jsonContent(GamesResponseSchema, 'List of all games'),
