...
export async function getDailyReport(startDate?: Date, endDate?: Date) {
  ...
  try {
    ...
    const ggrResult = await db
      .select({
        ggr: sql<number>`sum(case when ${transactions.type} = 'BET' then ${transactions.amount} else -${transactions.amount} end)`,
      })
      .from(transactions)
      .where(and(gte(transactions.createdAt, startOfDay), lte(transactions.createdAt, endOfDay)));
...
export async function getTokenReport(
  ...
  const result = await db
    ...
    .where(
      and(
        gte(transactions.createdAt, start),
        lte(transactions.createdAt, end),
        sql`${transactions.currencyName} IS NOT NULL`,
      ),
    )
  ...
  combined.sort(
    (a, b) => (a[sortKey as keyof typeof a] > b[sortKey as keyof typeof a] ? 1 : -1) * sortOrder,
  );
