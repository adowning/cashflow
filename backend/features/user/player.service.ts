import db, { type Player } from '@@/database';
...
export function getVipInfo(userId: string) {
  ...
  return db.select({ vipInfo: players.vipInfoId }).from(players).where(eq(players.id, userId));
}
...
export function setUserRTGSettings(time: number, userId: string) {
  console.log(time);
  console.log(userId);
  return db.update(players).set({ rtgBlockTime: time }).where(eq(players.id, userId)).returning();
}

async function getPlayerFavoriteGame(): Promise<string | undefined> {
 ...
 */
async function getPlayerBiggestWin(): Promise<number | undefined> {
