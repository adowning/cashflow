...
import { getGeoInfo } from '@@/lib/utils/getCountry';
import { getIpAddress } from '@@/lib/utils/utils.v2';
...
export const updateUsername = catchAsyncV2(async (c: Context) => {
  ...
  if (updateData.username) {
    if (await userService.usernameTaken(updateData.username, String(user._id))) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Username already taken');
    }
  }
  const updatedUser = await userService.patchUpdate({ _id: user._id }, updateData);
...
export const getPlayerTransactions = catchAsyncV2(async (c: Context) => {
  const user = c.get('user');
  const transactions = await transactionService.getPlayerTransaction(user._id, await c.req.json());
...
export const claimBonus = catchAsyncV2(async (c: Context) => {
  ...
  if (storedSocketId) {
    (global as any).io.to(storedSocketId).emit('balance', { amount: balance.amount });
  }
