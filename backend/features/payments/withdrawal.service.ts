...
import { canUserWithdraw } from '../player/wagering.service.js';
...
export async function requestWithdrawal(request: WithdrawalRequest): Promise<WithdrawalResponse> {
  try {
    ...
    if (!withdrawalEligibility.canWithdraw) {
      const errorMessage =
        'Withdrawal not allowed. Please complete bonus wagering requirements first.';
      ...
      return {
        ...
        validationErrors: withdrawalEligibility.blockingRequirements.map(
          (req: { id: any; requiredWagering: number; currentWagering: number }) =>
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      const existingMetadata = withdrawal.metadata
        ? JSON.parse(withdrawal.metadata.toString())
        : {};
      await tx
        .update(withdrawals)
        .set({
...
export async function getWithdrawalStatistics(days: number = 30): Promise<{
  ...
  try {
    ...
    const totalAmount = stats.reduce(
      (sum: any, stat: { amount: any }) => sum + (stat.amount || 0),
      0,
    );
