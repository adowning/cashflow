...
export async function seedVipLevels() {
  ...
  //   .from(players)
  //   .leftJoin(vipInfo, sql`${players.id} = ${vipInfo.playerId}`)
  //   .where(sql`${vipInfo.playerId} IS NULL`);

  // console.log(`Found ${usersWithoutVipInfo.length} users without VIP info`);

  // Generate and insert vipInfo for each user
  const vipInfoRecords = usersWithoutVipInfo.map((user) => generateRandomVipInfo(user.id));
  ...
  for (const record of vipInfoRecords) {
    ...
    await db.update(players).set({ vipInfoId: record.id }).where(eq(players.id, record.playerId));
  }
