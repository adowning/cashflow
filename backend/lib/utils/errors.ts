import { notifyError } from '../services/realtime-notifications.service';
 ...
 */
function identifyErrorCode(error: Error | string, context?: Record<string, any>): BetErrorCode {
...
export async function handleGameProviderTimeout(
  ...
  return new Promise((resolve) => {
    ...
    const timeoutId = setTimeout(async () => {
      const errorHandling = await handleBetError(new Error('Game provider timeout'), userId, {
        gameId,
        timeout,
      });
 ...
 */
export async function handleWebSocketError(userId: string, error: Error): Promise<void> {
