 ...
 * Now includes processing time capture for performance metrics
 */
export async function logTransaction(transactionData: TransactionLogData): Promise<string> {
  try {
    ...
    const transactionRecord = {
      ...
      beforeAmount: transactionData.preRealBalance + transactionData.preBonusBalance,
      afterAmount: transactionData.postRealBalance + transactionData.postBonusBalance,
      ...
      processingTime: transactionData.processingTime, // Add processing time tracking
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    ...
    // Insert BET/BONUS transaction with status
    await db.insert(transactions).values({
      ...transactionRecord,
      status: 'COMPLETED', // All bet transactions are completed when logged
    });
    ...
    throw new Error(
      `Failed to log transaction: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
}
  ...
  const detailedLog = {
    ...
    balanceChanges: {
      realBalance: {
        ...
        change: transactionData.postRealBalance - transactionData.preRealBalance,
      },
      bonusBalance: {
        before: transactionData.preBonusBalance,
        after: transactionData.postBonusBalance,
        change: transactionData.postBonusBalance - transactionData.preBonusBalance,
      },
    },
 ...
 */
async function calculateTransactionSummary(whereClause?: SQL): Promise<TransactionSummary> {
  ...
  const averageBet = totalTransactions > 0 ? totalWagered / totalTransactions : 0;
