 ...
 */
export async function validateBet(request: BetValidationRequest): Promise<BetValidationResult> {
  try {
 ...
 */
async function validateUserSession(userId: string): Promise<BetValidationResult> {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    session = await db.query.gameSessions.findFirst({
      where: and(eq(gameSessions.playerId, userId), gte(gameSessions.expiredTime, new Date())),
    });
  }
 ...
 */
async function validateGameSession(request: BetValidationRequest): Promise<BetValidationResult> {
 ...
 */
async function validateGameLimits(request: BetValidationRequest): Promise<BetValidationResult> {
  const game = await db.query.games.findFirst({
 ...
 */
async function validateWagerAmount(request: BetValidationRequest): Promise<BetValidationResult> {
 ...
 */
async function validateDailyLossLimit(request: BetValidationRequest): Promise<BetValidationResult> {
  ...
  const todayStats = await db
    ...
    .from(gameSessions)
    .where(and(eq(gameSessions.playerId, request.userId), gte(gameSessions.createdAt, startOfDay)));
...
async function validateSessionLossLimit(
  ...
  const sessionLosses =
    ((gameSession as any).totalWagered || 0) - ((gameSession as any).totalWon || 0);
 ...
 */
async function validateBalance(request: BetValidationRequest): Promise<BetValidationResult> {
  // Get user's active wallet
  // const user = await db.query.players.findFirst({
  //     where: eq(players.id, request.playerId),
  //     with: {
