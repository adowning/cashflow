...
import { logGGRContribution } from '../../../features/shared/ggr.service';
import { getJackpotPools, processJackpotContribution } from './jackpot.service';
import {
  notifyError,
  sendPostBetNotifications,
} from '../../../features/shared/notifications.service';
import { logTransaction } from '../../../features/shared/transaction.service';
import { addXpToUser, calculateXpForWagerAndWins, getVIPLevels } from '../vip/vip.service';
import { updateWageringProgress } from '../player/wagering.service';
...
export async function processBet(
  ...
  try {
    ...
    const finalBalances = await getBalance(usersWallet.walletId);
    if (!finalBalances) {
      throw new Error('Failed to retrieve final wallet balance for transaction logging');
    }
...
async function checkWalletService(): Promise<boolean> {
  try {
    ...
    if (!wallets || wallets.length === 0) {
      console.error(
        'Wallet service check failed: No wallets returned for test user (possible service or data issue)',
      );
      return false;
    }
    ...
    console.error(
      'Wallet service check failed: Exception during call -',
